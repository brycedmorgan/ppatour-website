...
import type { Availability } from "@/lib/vacations/capacity";
import { logo, scarcityThreshold } from "@/lib/vacations/content";
 ...
 * itinerary, stay + excursions, pros, pricing, getting there, calendar, closing
 * CTA. Rendered for every trip that is on sale: /vacations (Turks & Caicos)
 * and /vacations/trips/cancun both render this with their own content.
 *
 * Extracted from app/vacations/page.tsx on 2026-09-22 when Cancún went on sale
 * alongside Turks. Until then the page WAS the Turks trip, with "Grace Bay"
 * in its headlines; every one of those strings is now a `content.copy` field.
 *
 * Prices come from `cfg` (trip-config.ts) — the same object the checkout route
 * charges from — never from content, so a card can't show a number Stripe
 * won't charge. Rooms left come from `availability`, read by the page.
 */
...
export function TripPage({ content, cfg, availability }: TripPageProps) {
  const {
    trip,
    soldOut,
    copy,
  ...
  const [heroLine1, heroLine2] = copy.heroHeadline.split("\n");
  const registerHref = (occupancy?: Occupancy) =>
    `/vacations/register/?trip=${cfg.slug}${occupancy ? `&occupancy=${occupancy}` : ""}`;
      ...
      <section className="relative isolate overflow-hidden bg-ppa-navy-deep text-white">
        ...
        <div className="relative mx-auto w-full max-w-3xl px-4 py-16 text-center sm:py-20">
          ...
          <h2 className="mt-7 font-display text-2xl uppercase leading-[1.02] sm:text-4xl">
            {bookingClosed ? soldOut.headline : copy.closingHeadline}
          </h2>
