  ...
  return (
    ...
    >
      ...
      <Slide index={4} active={i} dir={dir}>
        <ScaleSlide active={i === 4} />
      </Slide>
      <Slide index={5} active={i} dir={dir}>
        <ApolloSlide active={i === 5} />
      </Slide>
...
// Eyebrow numbers are auto-generated by a CSS counter on `.usap-slide` (see
// DeckStyles) so re-ordering slides renumbers them automatically. Any `no`
// prop still passed at a call site is ignored.
function Eyebrow({ children, red }: { no?: string; children: React.ReactNode; red?: boolean }) {
  return (
    <p className="usap-eyebrow" style={red ? { color: USAP_RED } : undefined}>
      <span className="usap-eb-no" aria-hidden />
      {children}
    </p>
  );
}
...
const UNITS = [
  {
    ...
    tone: "sky",
    body: "Carvana PPA Tour, Major League Pickleball, and PickleballTV — the events and the broadcast that carry the sport.",
  ...
  },
  {
    k: "Technology",
    tone: "red",
    body: "Pickleball.com — Tournaments, Leagues, Team Leagues, and Clubs. The digital backbone the whole sport runs on.",
    stat: "95%+ of U.S. tournaments · 2.9M+ matches run",
  },
  {
    k: "Other",
    tone: "red",
    body: "Just Courts, The Picklr, and DUPR — where the game gets built, played, and rated nationwide.",
...
const TRAJ = [
  { num: "$150", unit: "M", lbl: "Revenue this year" },
  { num: "$300", unit: "M", lbl: "In five years" },
  { num: "$1.5", unit: "B", lbl: "Enterprise value" },
];

function PlanSlide() {
  return (
    <div className="usap-wrap">
      <Eyebrow no="06">Where We&rsquo;re Headed</Eyebrow>
      <h2 className="usap-h2-center">The five-year plan.</h2>
      <div className="usap-traj">
        {TRAJ.map((t, idx) => (
          <div className="usap-traj-row" key={t.lbl}>
            <div className="usap-traj-item">
              <div className="usap-traj-num">
                {t.num}
                <span>{t.unit}</span>
              </div>
              <div className="usap-traj-lbl">{t.lbl}</div>
            </div>
            {idx < TRAJ.length - 1 && (
              <span className="usap-traj-arrow" aria-hidden>
                →
              </span>
            )}
          </div>
...
.usap-deck{
  ...
  counter-reset:usapslide -1; /* title = 0 (no eyebrow), Moment = 01, ... */
...
.usap-eb-no::before{content:counter(usapslide, decimal-leading-zero)}
.usap-eyebrow::after{content:"";flex:0 0 auto;width:clamp(28px,5vw,72px);height:1px;background:rgba(159,180,214,.34)}

/* Pickleball Inc wordmark (text stand-in for the official logo) */
.usap-pbinc{font-family:var(--font-gotham);font-weight:900;text-transform:uppercase;letter-spacing:-.015em;
  color:#fff;white-space:nowrap;line-height:1;display:inline-flex;align-items:baseline;gap:.14em;
  font-size:clamp(28px,4vw,56px)}
.usap-pbinc em{font-style:normal;color:#4DC1EF;font-size:.62em;letter-spacing:0}
.usap-plate--sm .usap-pbinc{font-size:clamp(15px,1.7vw,22px)}
.usap-signoff-pbinc{font-size:clamp(15px,1.7vw,22px)}

/* five-year trajectory */
.usap-traj{display:flex;align-items:flex-start;flex-wrap:wrap;gap:clamp(8px,1.6vw,22px);
  margin-top:clamp(22px,3.6vh,40px)}
.usap-traj-row{display:flex;align-items:center;gap:clamp(8px,1.6vw,22px)}
.usap-traj-item{display:flex;flex-direction:column}
.usap-traj-num{font-family:var(--font-gotham);font-weight:900;line-height:.9;letter-spacing:-.02em;
  font-size:clamp(30px,4.6vw,60px);color:#fff}
.usap-traj-num span{color:#E7C079;font-size:.5em;margin-left:.03em}
.usap-traj-lbl{text-transform:uppercase;letter-spacing:.1em;font-size:clamp(9px,1vw,12px);color:#9FB4D6;margin-top:.6em}
.usap-traj-arrow{font-family:var(--font-gotham);font-weight:900;color:#4DC1EF;font-size:clamp(20px,2.6vw,34px);line-height:1;
  position:relative;top:.15em}
@media (max-width:860px){
  .usap-traj-num{font-size:clamp(26px,7vw,40px)}
}
...
/* title */
.usap-title-h1{font-size:clamp(50px,9.8vw,168px);margin-top:clamp(38px,7vh,92px)}
