...
function SideRow({
  ...
  return (
    <div className="flex min-h-[1.75rem] items-stretch">
      ...
      {/* ⚠ A WALKOVER WINNER GETS THE SAME TREATMENT AS EVERY OTHER WINNER, and
          it needs its own cell to get it: the green highlight lives on the SCORE
          cells and a withdrawal has none, so the side that advanced was styled
          like the side that lost. One cell in the same column carries "W" for the
          team that went through and a dash for the team that withdrew.
          Rendered even when `showScores` is false — the round may be unplayed
          apart from this walkover, and the result still happened. */}
      {(showScores || outcome === "walkover") && (
        <span className="flex shrink-0 items-stretch border-l border-ppa-line">
          {(outcome === "walkover"
            ? [side.winner ? "W" : "–"]
            : (games.length ? games : [null, null, null]).map((g) => g ?? "")
          ).map((cell, i) => (
            <span
              key={i}
    ...
    >
      ...
      {hasFooter && (
        <div className="flex items-center gap-1.5 border-t border-ppa-line px-2 py-0.5 text-[10px]">
          ...
          {/* Decided without being played — otherwise the card shows two names,
              no score and a highlight on one side, with nothing explaining it. */}
          {m.outcome === "walkover" && (
