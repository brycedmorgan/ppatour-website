import { capacity, trip } from "./content";
 ...
 * behind writes). On a 20-room block selling a few rooms a week that's
...
/**
 * The room block Lainey maintains in Jackalope. She contracts rooms with the
 * resort and adjusts them there; keeping the number in this repo too would mean
 * a code deploy every time the block changes, and a silent oversell the first
 * time the two disagree. `content.ts` capacity stays as the fallback so a
 * Jackalope outage can't take the pricing cards down.
 */
const PLAN_URL =
  process.env.JACKALOPE_PLAN_URL ??
  "https://pickleball.usejackalope.com/api/public/vac-plan";

async function contractedRooms(): Promise<Record<Occupancy, number>> {
  try {
    const res = await fetch(
      `${PLAN_URL}?destination=${encodeURIComponent(trip.destination)}`,
    ...
    const json = (await res.json()) as { capacity?: Record<string, unknown> };
    const out = { ...capacity };
    for (const id of Object.keys(capacity) as Occupancy[]) {
...
function build(
  ...
  known: boolean,
  contracted: Record<Occupancy, number> = capacity
): Availability {
  ...
  for (const id of Object.keys(PRICING) as Occupancy[]) {
    ...
    options[id] = { total, sold: n, left, soldOut: known && left === 0 };
  }
  return {
    options,
    tripSoldOut: known && Object.values(options).every((o) => o.soldOut),
    known,
  };
}
...
async function countSoldRooms(): Promise<Record<Occupancy, number>> {
  const stripe = getStripe();
  const sold = emptyCounts();
  const query = `status:"succeeded" AND metadata["destination"]:"${trip.destination}"`;
...
/**
 * Rooms left per occupancy. Never throws: if Stripe is unreachable we return
 * `known: false` and the caller falls back to open booking. Failing closed
 * here would take the whole trip off sale over a transient Stripe blip, which
 * is a far worse outcome than briefly overselling by one room.
 */
export async function getAvailability(): Promise<Availability> {
  if (cache && Date.now() - cache.at < CACHE_MS) return cache.value;

  const contracted = await contractedRooms();
  if (!stripeReady()) return build(emptyCounts(), false, contracted);

  let value: Availability;
  try {
    value = build(await countSoldRooms(), true, contracted);
  } catch (err) {
    console.error("[capacity] Stripe availability lookup failed", err);
    return build(emptyCounts(), false, contracted);
  }
  cache = { at: Date.now(), value };
  return value;
}

/** Drop the cache so the next read reflects a booking that just completed. */
