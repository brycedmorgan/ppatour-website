...
| Item | State | Blocker |
|---|---|---|
| **Contact-form triage** | Built 9/17: Claude classifies every contact submission, routes tickets/volunteer/media/registration/careers to their inbox + channel, answers event-info / volunteer / rankings questions from the site's own facts, threads the answer in #ppa-marketing-form and ✅s it. LIVE 9/17 with `FORM_TRIAGE_REPLY=off` (answers visible in Slack + sheet, no fan email yet). [`FORMS.md`](FORMS.md) § Contact-form triage | read the first real answers in the channel threads → replay the 38 real questions with `scripts/triage-eval.ts` → decide when `FORM_TRIAGE_REPLY` goes on · `reactions:write` on the Slack app (Wesley) · then fill the site gaps `triageOpen` keeps naming |
