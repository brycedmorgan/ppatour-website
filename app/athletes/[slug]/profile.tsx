...
/**
 * Which site's chrome this profile renders inside. "europe" is the same page
 * mounted at /europe/athletes/[slug] (Payton Pemberton, 9/22: a Europe pro's
 * card opened the US page, Carvana header and all). It changes only what LEADS
 * OFF the page — redirects, More Pros, the rankings and watch links, the title
 * suffix. The profile content is identical, and the canonical stays
 * /athletes/[slug]: one athlete is one page to a search engine.
 */
export type ProfileRegion = "us" | "europe";


export async function athleteStaticParams() {
...
export async function athleteMetadata(
  slug: string,
  region: ProfileRegion = "us",
): Promise<Metadata> {
  ...
  if (europe) description = description.replaceAll("Carvana PPA Tour", "PPA Tour");
  /**
  ...
  return {
     ...
     */
    robots:
      isUnlistedEuropeAthlete(slug) || (europe && europeRobots)
...
export async function AthleteProfile({
  ...
  const europe = region === "europe";
  const base = europe ? "/europe/athletes" : "/athletes";
  /**
  ...
  if (toCanonical[slug]) permanentRedirect(`${base}/${toCanonical[slug]}`);
   ...
   */
  if (!a) redirect(europe ? "/europe" : "/athletes");
  ...
  const others = ((): { slug: string; name: string; headshot: string }[] => {
    ...
    /**
     * On the Europe mount, More Pros is the Europe roster — the WPR window
     * above would send a Europe visitor to US pros and, through them, back
     * into the US site. Next four in roster order, wrapping, portraits only.
     */
    if (europe) {
      const withFace = europeRoster.filter((p) => p.portrait);
      const i = withFace.findIndex((p) => p.slug === a.slug);
      return [...withFace.slice(i + 1), ...withFace.slice(0, Math.max(i, 0))]
        .slice(0, 4)
        .map((p) => ({ slug: p.slug, name: p.name, headshot: p.portrait! }));
    }
    if (picks.length) {
      return picks.map((p) => ({
  ...
  return (
    <>
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(
            breadcrumbJsonLd([
              ...(europe
                ? [
                    { name: "PPA Tour Europe", path: "/europe/" },
                    { name: a.name, path: `/europe/athletes/${a.slug}/` },
                  ]
                : [
                    { name: "Home", path: "/" },
        ...
        <div className="mx-auto w-full max-w-6xl px-4 py-12">
          <div className="grid gap-10 lg:grid-cols-[1.4fr_1fr]">
            <div>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2">
                ...
                {highlights ? (
                  ...
                  <Link
                    href={europe ? "https://www.youtube.com/@PPATourEurope" : "/watch"}
                    className="inline-flex h-11 items-center bg-ppa-blue px-6 text-xs font-bold uppercase tracking-[0.12em] text-white transition-colors hover:bg-ppa-blue-deep"
                  >
                    ▶ Where to Watch
                  </Link>
            ...
            <aside>
              ...
              <Link
                href={europe ? "https://pickleball.com/rankings" : "/rankings"}
      ...
      {/* Email — the US fan list, so not on the Europe mount. */}
      {!europe && (
