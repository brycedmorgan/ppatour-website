 ...
 *   npm run engine:properties -- --csv     # the same hotels as a fill-in sheet
 ...
 */
// --csv is a flavour of --list, so it must not also require --list to be passed.
const LIST = args.includes("--list") || args.includes("--csv");
/**
 * Emit the hotel list as CSV on stdout instead of the human-readable block —
 * a fill-in sheet for whoever holds the Engine login, with a blank ID column.
 *
 * ⚠ IT PRINTS RATHER THAN COMMITTING A FILE, ON PURPOSE. The hotel rows come
 * from Kristen's live Jackalope feed, so a sheet checked into the repo starts
 * going stale the day a block moves. Redirect it when you need to send one:
 *
...
async function main() {
  if (LIST) {
    ...
    if (CSV) {
      // Minimal RFC-4180 quoting: hotel names and addresses both carry commas
      // ("Hotel Ivy, Luxury Collection", every street line).
      const cell = (v) => `"${String(v ?? "").replace(/"/g, '""')}"`;
      console.log(
        ["Event", "Hotel", "Where Engine should look", "Engine property ID (fill in)", "Key — leave alone"]
          .map(cell)
          .join(","),
      );
      for (const h of unique) {
        console.log([h.source, h.name, h.query, "", norm(h.name)].map(cell).join(","));
      }
      process.exit(0);
    }
