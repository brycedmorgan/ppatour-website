...
### 2026-09-22 (pt. 4) — The Challenger board is the event team's own workbook now; the "About" tail pointed at this page
...
- **WPR: 3 of the 10 "Unranked" were a SLUG MISMATCH, not missing data** (corrected after Payton: Raquel has 9 pts from Portorož). The rankings API keys Raquel Amaro as `raquel-amaro-veloso`, Ellie Tomkinson as `ellie-tomkinson-1`, James Ling as `james-ling-2` — not their profile slugs. Fixed with a lookup-only `rankSlug` on `lib/europe-roster.ts` → `EUROPE_RANK_SLUGS`, read by `rankings-api` SLUG_ALIAS, `division-rankings` and the /europe roster. ⚠ NOT `CURATED_TO_CANONICAL` — that map also drives page slugs/sitemap and would de-list the indexed `-veloso`/`-2` URLs and swap in scraped bios (QA gate caught it). Found via a throwaway preview route matching the live board by NAME. The other 7 (Karbownik, O'Donoghue, Dahlin, Jiménez, Rodrigues, Alvarez, Campos) are genuinely not on the board. ⚠ A new Europe signing's rank slug must be checked by name, not assumed from the profile URL.
...
- The event team's submitted copy, pasted verbatim into `PARKING_BY_SLUG` with their map. Vegas was on the `PARKING_TBA` holding line until now, and the stop starts in six days. **Third finalized stop of the season** (Cary 8/5, Arizona 9/10 + 9/15).
- **⚠ RIDESHARE IS INSIDE GENERAL PARKING, NOT ITS OWN SECTION.** Cary and Arizona both carry a `Rideshare` heading, so the tidy-looking move is to lift the sentence out into one here. They wrote it as the middle paragraph of general parking — filing a sentence under a heading they did not write is the same class of edit as rewording it, and the 8/5 pass deleted 18 strings to close exactly that. Two sections, not three.
- **⚠ NO PREMIUM PARKING SECTION, AND THAT SILENCE IS LOAD-BEARING FOR THE CONCIERGE.** Their submission says on-site, first-come first-served, and names no paid lot or pass. The concierge's "Premium parking on Tixr" button is gated on `/premium parking/i` in the copy, so it correctly does **not** attach here even though tickets are on sale and `parkingPassUrl` resolves — verified in the rendered facts payload. Cary sells one; Vegas, on what we have been told, does not.
- **⚠ THE TOW WARNING IS THE ONLY LINE ON THE SITE THAT THREATENS A CONSEQUENCE, AND IT POINTS AT THE MAP.** *"Parking in the drop-off/pickup loop shown in red on the map below is strictly prohibited and will be enforced by towing."* So the map ships in the General Parking section, under that paragraph — the words and the thing they point at have to arrive together (same call as Cary's "refer to the attached map"). Supplied art, 2000×636 webp @ q72, 195 KB, in line with Arizona's 223 KB.
- **⚠ IT IS A 3.14:1 STRIP — FAR WIDER THAN THE OTHER TWO MAPS (1.45:1, 1.37:1) — AND LEGIBILITY WAS CHECKED BY LOOKING, NOT BY ARITHMETIC.** It draws 324×103 in the phone accordion, i.e. the source scaled to 0.13×, which on paper reads as unusable. Rendered at that exact size it is fine: the three yellow lots, the red loop, "Darling Tennis Center" and "W Washington Ave" all still read, because every marked feature is large. **No crop** — a lot outlined on a satellite photo is the claim, and trimming context is how a driver ends up in the soccer complex.
- ⚠ The concierge repeats "shown in red on the map below" in a bubble that shows no map. Pre-existing and consistent — `parkingText()` deliberately omits images, and Cary ("refer to the attached map") and Arizona ("See map below") both do the same. Rewording it for the bubble would break the verbatim rule.
- Verified on rendered pages at 1440 and 390, not by grep over source: **both surfaces** on the event page (KBYG accordion + Plan Your Trip card) and the `/today` screen in its dark tone, **0 occurrences of the holding line**, map loaded and `complete` in all five renders, **0 horizontal overflow at both widths**. Search finds the stop by *darling*, *towing* and *first-come*. **Control: Chicago Cup still renders the holding line ×2 and 0 Vegas map.** tsc clean (the two known scratchpad probes aside), eslint clean, `next build` green.
- ⚠ **Still open at this stop, carried from 9/20: gates are the template's** (8/9/10 AM against a 2 PM mid-week first serve, and EQUAL to first serve on Mon and Sun). This submission did not include gate times. Las Vegas remains the stop most in need of a `GATES_BY_SLUG` line.
...
- **Bryce's WP-admin exports landed 9/21 night** (WXR media export + 7 Gravity Forms CSVs), filed in the archive under `wp-export/` and `gravity-forms/`. The WXR lists all **303** attachments; the 113 the API hid were pulled by pinned IP, so **the media library is complete** (303 of 303, 34 MB originals, manifest with title/alt/caption/date from the WXR).
- **⚠ THE FORMS ARE THE MAIN SITE'S TOO.** The Challenger site was cloned from the ppatour.com WordPress in July 2023, so every form carries main-site entries from 2023-07-12. `gravity-forms/README.md` has the table. Three things needed action, not filing:
  - **Sponsorship inquiries:** 407 rows; **76 from 2024-05 → 2024-09 (+1 Jan 2026) fill part of the gap** in Jackalope's `website-archive` (9/18). Plus **5 real Challenger "Become a Sponsor" leads** 2025–26 (PickleRage, Premium Care Chiropractic, JLB/true-wave, Good Feet Store, Invincible Supplements). Deduped against `deals` by contact email + company: **15 already there, 66 new** → `~/pickleball/_private/challenger-site-sponsorship-inquiries-NEW-66.csv` and a ready `…-import.sql` (same shape as the 9/18 import: stage hold, source website-archive). **NOT RUN** — the 9/18 memory says the CRM bulk write only runs on Bryce's explicit go. Oct 2024 → Aug 2026 is STILL missing (that run lived in the separate ppatour.com WP, not the clone).
  - **Contact form: 25 questions since 2026-08-01, 12 about Charlotte / Grand Rapids registration dated Sept 10–20**, nobody was being notified. Extracted to a CSV for Tanner; the Slack file upload was denied by the auto-mode classifier (PII), so Bryce forwards it.
  - Lists: 3,786 unique "notify me" emails (2023–25 main-tour stops), 1,191 Junior PPA newsletter signups (2023-12 → 2024-09). Stale; noted, not actioned.
- Still open: Tanner's 2026 Showdown dates + per-stop points; a live rankings source; internal Challenger event pages for 1:1 post redirects; Flywheel cancel (now safe).
...
- **Built as a new `#showdown` section on `/tour/challenger` AND as days inside the Worlds event page's own order of play.** Both read **new `lib/challenger-showdown.ts`**. Nothing is retyped on either page: a qualifying count that disagrees with itself across two surfaces is this repo's most familiar bug, and these are the rules a player plans a season around.
- **⚠ THE SLUG ALONE WOULD HAVE ADVERTISED A 2027 CONTRACT ON LAST YEAR'S EVENT PAGE, and only the live feed showed it.** `pickleball-world-championships` is carried **twice** — the completed **2025** edition (Nov 3–9, 2025) and the 2026 one — because annual editions share a slug here, and `/events/[year]/[slug]` renders a page for each. The first draft gated the callout on the slug. `showdownDaysFor()` now takes the event's own window and returns its days only when **all** of them fall inside it. **Verified: `/events/2025/…` renders 0 mentions, `/events/2026/…` renders 11.**
- **⚠ AND THAT GATE MAKES THE DERIVED DATES SELF-CHECKING, WHICH IS THE ONLY REASON THEY ARE PUBLISHED AT ALL.** Her request has a **"When" heading with nothing under it**. What it does give is the weekday of each block — singles pool play Thursday, doubles Friday, semis and finals Saturday — and Worlds is **Nov 2–8, 2026**, which contains exactly one Thursday, Friday and Saturday. So Nov 5/6/7 is the only reading of her own copy, not a plausible week picked to fill a gap. **Confirmed against the LIVE feed, not the curated row** (`source: live`, `2026-11-02 → 2026-11-08`, Brookhaven). If Worlds ever moves, the days stop falling inside the window and both surfaces go quiet rather than publishing a stale week. **Still ask Brooke to confirm.**
- **⚠ ONE RULE IS DELIBERATELY WITHHELD.** Her closing line gives "players will be permitted to compete in multiple disciplines" to **"the 2027 Challenger Showdown"**, while the body of the same request describes **"the 2026 Challenger Showdown"** awarding contracts for 2027. A player enters a second draw on an eligibility rule; publishing it under the wrong year costs somebody an entry. One line in `challenger-showdown.ts` the moment she says which edition it is.
- **⚠ NEW `SIDE_EVENTS_BY_SLUG` IN `lib/event-schedule.ts`, AND IT EXISTS BECAUSE THE ALTERNATIVE WAS INVENTING SEVEN FIRST-SERVE TIMES.** Everything with a published day belongs on that day (Bryce, 7/31: one calendar block) — but **only Nationals has an `eventSchedules` entry**, so Worlds renders the template, which has no Amateur & Junior column to put a session in. Writing a full entry to hang three sessions off it would have meant supplying a gate and a first serve for every day of the event. Same reasoning as `GATES_BY_SLUG`, same ISO-date keying, and the same ⚠: a stop that later gains a real entry **stops reading this map**, so its companion events must be carried into that entry's `amateur` arrays in the same commit.
- **⚠ THE HOST EVENT IS NAMED FROM THE FEED, NOT FROM THE REQUEST.** She writes "UPA OpenDoor World Championships"; the feed — the source of truth for event names since 8/3 — says **Opendoor Pickleball World Championships**, which is what every other surface prints. The Challenger page's link to it is resolved from that same feed by window, not slug, so it points at `/events/2026/…` and never last year's.
- Both copies of `buildSchedule` carry `Day.side` (event page + `NationalsLive`) — they drift silently, and Nationals hosts none, so the second one is inert by design.
- Verified on rendered pages at 1440 and 390, not by grep over source: the section reads **Nov 5–7, 2026**, all three day rows, all three qualifying lines and the Oct 26 partner deadline; the Worlds page carries the Showdown on **Nov 5, 6 and 7** in the table plus an "Also this week" callout linking to `#showdown`. **0 horizontal overflow on both pages at both widths, and 0 of the wider-than-viewport elements are in the new sections** — all of them are the pre-existing `overflow-x-auto` tables and the Ken Burns hero. **Controls unchanged: 2025 Worlds, Las Vegas (templated) and Nationals (`eventSchedules`) all render 0 mentions.** tsc clean, eslint at the NationalsLive `set-state-in-effect` baseline, `next build` green.
- ⚠ Method, both already documented and both hit again: `next build` needs `scratchpad/probe-platform-denied.ts` **and `probe-rounds.ts`** moved aside — either one fails the whole typecheck — and `BUILD_DIST_DIR=.next-buildcheck` appends two entries to `tsconfig.json` (reverted). `wpr-snapshot.json` was rewritten with identical data and reverted.
- **Open, both with Brooke:** the Showdown's own dates · which edition the multiple-discipline rule belongs to.

### 2026-09-20 (pt. 2) — Las Vegas' official per-day start times; the “hour before first serve” claim breaks from the other side too

- The event team, 9/20: *“change the times for Las Vegas. First serve at 2pm.”* Then, the same afternoon, the official per-day list — **Mon 8am · Tue–Fri 2pm · Sat 1pm · Sun 10am**. Three of the seven days are not 2pm, so it is transcribed day by day in `FIRST_SERVE_BY_SLUG`. Shipped `edad031`.
- **⚠ THE FLAT “2PM” WOULD HAVE STARTED CHAMPIONSHIP SUNDAY AS ITS BROADCAST ENDED, and the sheet is what caught it before it shipped.** `node scripts/audit-tv-schedule.mjs` pulls the LIVE sheet, and its 9/10 revision had already moved Las Vegas into the evening exactly as it did Arizona. Nevada is Pacific, so ET is local + 3 in October: Thu 5PM ET and Fri 5PM ET = **2PM local** ✓, Sat 4PM ET = **1PM** ✓, Sun PBTV 1PM ET and TC 1PM–5PM ET = **10AM–4PM** ✓. **Every day the sheet covers agrees with the official list independently** — two sources, same answer. A flat 2pm on Sunday would have begun as the Tennis Channel window closed.
- ⚠ **Monday is 8am and is not a typo** — it is the amateur and junior bracket day, which has nothing to do with the evening pro sessions after it.
- **⚠ THE “GATES OPEN AN HOUR BEFORE FIRST SERVE” LINE BREAKS FROM THE FIRST-SERVE SIDE TOO, WHICH THE 9/18 GATES WORK DID NOT COVER.** That claim is only true where **both** times are the template's, because the template derives the gate an hour before its own first serve. A supplied GATE already suppressed it (Arizona, 9/18); a supplied FIRST SERVE breaks it just as thoroughly, and Las Vegas keeps the templated gate — so the page would have claimed an hour while the table beside it showed five. New **`gatesFollowFirstServe()`** replaces the `hasGatesOverride` test on the schedule intro **and** on the concierge's schedule answer, which carried the same sentence. A stop in that state now reads *“Gate times are listed per day below.”*
- **⚠ GATES ARE STILL THE TEMPLATE'S AT THIS STOP AND IT NOW LOOKS WORSE, NOT BETTER (Wesley's call, made before the per-day list arrived).** Nobody has sent a Las Vegas gate time, so the table reads a templated 8/9/10 AM gate against these — five hours early mid-week, and **EQUAL to first serve on Mon and Sun**, where the template happens to land on the same number. “Gates 10:00 AM · First serve 10:00 AM” reads as a statement rather than a placeholder. **This is the stop most in need of a `GATES_BY_SLUG` line.**
- Verified on rendered pages: Las Vegas reads **8:00 AM / 2:00 PM ×4 / 1:00 PM / 10:00 AM**, **0 occurrences of the hour-before claim**, and the concierge names each day's own time. **Controls unchanged — Arizona still “Gates open 12:00 PM daily”, Chicago Cup and the Nationals `-live` route still carry the hour-before line.** tsc clean, eslint at the NationalsLive baseline, `next build` green.
- **⚠ FOUND, NOT FIXED: our Las Vegas BROADCAST windows are stale against the same 9/10 sheet.** The audit reports three: **Thu Oct 1 PBTV 1PM–9PM ET → 5PM–1AM**, **Fri Oct 2 1PM–9PM → 5PM–1AM**, **Sat Oct 3 12PM–8PM → 4PM–12AM**. The event team's own start times now corroborate the sheet, so the site is publishing the pre-reschedule windows on a stop eight days out. Needs `lib/broadcast.ts` and `lib/tv-schedule.ts` in lockstep. (The audit also flags **Nationals Sep 5** — that one is the documented 9/3 weather reschedule, where the site is deliberately ahead of the sheet. Leave it.)

### 2026-09-20 — The scores board picks by ROUND, off the order of play; the phantom bronze is gone

- Wesley, during the live stop: put a dropdown on Live & Latest and the event page's scores keyed to **the round the match is in, not the date** — then, on seeing it: **“show the specific round based off the order of play info for that tournament… today we would show the finals because the order of play says Championship Sunday - Finals.”** Both surfaces render the same `ScoresBoard`, so one component covered them. Shipped `835a022`.
- **⚠ THE DAY PICKER WAS WRONG BECAUSE THE DRAW DOES NOT RESPECT THE CALENDAR.** Measured on the live Arizona feed: **Women's Doubles Round 32 was played on BOTH Tue Sep 15 and Thu Sep 17**, Men's Singles Round 16 on Thu and Fri — so one round was **split across two dropdown entries** and a fan looking for the round of 32 had to know which half fell on which day. The reverse too: **Friday held three different rounds at once.**
- **⚠ ROUNDS ARE GROUPED BY THE FEED'S LABEL AND ORDERED BY A DERIVED DEPTH, NEVER KEYED ON `roundNumber`.** That field counts up from 1 at each DIVISION's own first round, so at a stop where one division draws 64 and another draws 32, `roundNumber` 1 is Round 64 in the first and Round 32 in the second — keying on it would file two different rounds under one entry and **print the wrong name over half of them.** Depth is `max(roundNumber in that division) - roundNumber`, which agrees across draw sizes. **Nothing reads a round NAME**, so there is no hardcoded “Quarter Finals before Semi-Finals” ladder to drift out of step with the feed.
- **⚠ THE DEFAULT COMES FROM THE ORDER OF PLAY, AND ON THE DAY THAT MATTERS THE SCHEDULE AND THE DATA DISAGREE.** At 8am on Championship Sunday nothing in the final has been played and the deepest PLAYED round is the semifinals — so a data-derived default opens the board on yesterday's business **on the one morning everybody arrives looking for the final.** New `lib/order-of-play.ts`; the round ladder **moved out of both copies of `buildSchedule`** into it, so the printed Order of Play table and the board cannot disagree about what today is. The map is built on the SERVER, the day read from the DEVICE's clock — the same split as the 8/31 qualifier switch.
- **Card headers now show the DAY.** The picker names the round, so repeating it per card spent the most prominent line on a card restating the current selection, while the day — which a round can straddle — had nowhere to appear at all.
- **⚠ BRONZE IS HIDDEN VIA THE FEED'S OWN FLAG, AND MATCHING ON THE NAME WOULD HAVE DELETED 26 REAL MEDAL MATCHES.** There is no third-place match at a 1,000-point stop (Connor, 7/23), but the draw still carries one row per division — so the board published a Bronze round of fixtures nobody would ever play: **5 at Arizona, 5 at Nationals, 5 at Kuala Lumpur, every one stuck at “Date TBA” forever.** Those come through as **`inBracketType: "HIDE"`**; the genuine ones at the **Shenzhen 500, Ho Chi Minh 500, Gold Coast 125 and the Seattle / Atlanta / Grand Rapids Challengers** come through as `B / Bronze` with real winners and dates. Dropping by name would also have **contradicted the Final Standings podium on the same page**, which lists a bronze medallist at those stops. **0 of the 27 `HIDE` rows sampled had ever been played**, so the filter cannot swallow a result; it also clears the 9–13 nameless `HIDE` rows a Challenger draw carries. One line in `normalize`, so it applies to every surface at once.
- **⚠ FOUND, NOT FIXED, AND IT IS A LANDMINE: the API names five Arizona champions while the finals are unplayed.** `standingsOf` falls back to **the deepest COMPLETED match** when no gold-medal match is done, so on Championship Sunday morning it crowned the **semifinal winners as champions and the semifinal losers as silver**. Nothing renders it — `ChampionsBanner` is gated on the event being `completed`, verified — so it is not live today. A guard built on `champions` was tried here and **removed** once this turned up; do not lean on that signal.
- ⚠ Also found: the Finals cards read **“Date TBA”** because the feed publishes no planned start for a final, on a day whose Order of Play says 2:00 PM. The feed's gap, not ours.
- Verified in a real browser, not by grep: Arizona, Nationals and the homepage all render **Round 64 → Finals with no Bronze and open on Finals**; completed Nationals reaches Finals through the fallback; **Men's Singles Round 16 shows Thursday's AND Friday's matches together** with per-card dates; round and division switching work; **0 horizontal overflow at 1440 and 390**. Podium untouched — Shenzhen keeps all five bronze medallists, Nationals still gold/silver only. 15/15 on the day-label→round matcher. tsc clean, eslint at the existing `set-state-in-effect` baseline, `next build` green (2,071 pages).
- ⚠ Method: `BUILD_DIST_DIR=.next-buildcheck` again appended two entries to `tsconfig.json` (reverted), and **`next build` rewrites `lib/data/wpr-snapshot.json`'s `generatedAt` with identical data** — revert it before committing after a build.

### 2026-09-19 — Arizona's first serve is 2PM Sat and Sun; the concierge and the phone were both silent on it

- Tyler Petersen and Parker Roberts, Slack, during the live stop: **first serve is 2:00 PM today and tomorrow.** Production was publishing the template's **10:00 AM Saturday and 11:00 AM Championship Sunday** against gates that open at noon. Shipped `e0c25e0`.
- **New `FIRST_SERVE_BY_SLUG` + `firstServeFor()` in `lib/event-schedule.ts`**, a sibling to yesterday's gates override and read by **both** copies of `buildSchedule`, so the table, the `-live` route and the concierge cannot disagree.
- **⚠ KEYED PER DAY, NOT PER STOP, AND THAT IS THE DIFFERENCE FROM THE GATES MAP.** Gates open at noon every day here; a first serve supplied for Saturday says nothing about Tuesday. So the map is slug → ISO date → time, and **Sep 14–18 deliberately have no line**: nobody sent times for them, they have been played, and a plausible 2:00 PM written across the week would be an invention wearing a correction's clothes. They still read the 9/10 AM placeholder. **The whole week needs a real `eventSchedules` entry, not more lines here.**
- **⚠ THE KEY IS DERIVED FROM THE EVENT'S OWN START DATE, so a date change silently un-matches it** and the day falls back to the template. Written on the constant; re-check the map whenever a stop moves.
- **⚠ THE CONCIERGE ANSWERS “first serve” AND WAS REPLYING WITH THE GATE TIME ALONE.** Its schedule intent matches `first serve|start|time`, and with yesterday's gates work it said only “Gates open 12:00 PM daily” — on the same page whose table now carries the real start. New required `ConciergeFacts.firstServeNote`, built by `firstServeNote()` **from the rendered rows rather than from the map**, so the bubble cannot name a day or a time the order of play does not. Null on every other stop, where naming a time would be inventing one.
- **⚠ AND ON A PHONE THE TEMPLATED TABLE NEVER SHOWED FIRST SERVE AT ALL.** That column is `sm:block`, so Arizona read “Gates 12:00 PM” and nothing else — i.e. the one fact the event team asked us to fix was invisible to most of the people reading it. It now carries the inline `· First serve X` treatment the `realSchedule` branch has always had. **This is the one change that touches every event page.**
- Verified on rendered pages at 1440 and 390, not by grep: Sep 19 and Sep 20 both read **2:00 PM** in the column and inline, **0 occurrences of the 11:00 AM placeholder**, gates still 12:00 PM ×7, **0 horizontal overflow**. **Controls unchanged — Las Vegas still 8/9/10 AM gates against 9/10/11 AM first serve with the hour-before claim intact, the Nationals `-live` route still on its own transcribed times with no note.** tsc clean, eslint at the NationalsLive `set-state-in-effect` baseline, `next build` green.
- ⚠ `next build` rewrites `lib/data/wpr-snapshot.json`'s `generatedAt` with identical board data. Reverted so it stayed out of the diff — worth knowing before committing after a build.
- **⚠ STILL OPEN, BOTH CARRIED OVER FROM 9/18:** `/today` shows this stop **no gate time and no first serve**, because `TodayPanel` reads `eventSchedules` and Arizona has no entry — that is the screen someone opens standing in the lot, and closing it needs the full week's times. And the Know Before You Go card still says **“Morning and evening sessions are ticketed separately”**, templated onto every event and almost certainly false for a stop that starts at 2PM.

### 2026-09-18 (pt. 4) — Arizona's gates open at noon; the template was publishing 9AM mid-tournament

- Wesley, during the live stop: the gates open at 12 PM. Production was publishing **8:00 AM, 9:00 AM and 10:00 AM** across the seven days.
- **⚠ THE WRONG TIMES WERE THE TEMPLATE'S, NOT A TYPO — this stop has no transcribed order of play.** Only Nationals has an `eventSchedules` entry, so every other stop renders `buildSchedule`'s house defaults: 8AM on the two lead-in days, 9AM through the week, 10AM on Championship Sunday. A fair stand-in for a daytime stop, and simply wrong for this one — the **9/10 broadcast sheet moved every Arizona window into the evening**, PBTV opening 5PM ET on all four days, which is **2PM local** (Arizona keeps no DST, so ET is local + 3 in September). A gate time is the one fact on that page that decides when a family physically arrives.
- **New `GATES_BY_SLUG` + `gatesFor()` in `lib/event-schedule.ts`**, read by **both** copies of `buildSchedule` (event page and `NationalsLive`), so the table, the `-live` route, the Know Before You Go line and the concierge cannot disagree. One line per stop. Filling in a full `eventSchedules` entry instead would have meant inventing seven first-serve times.
- **⚠ FIRST SERVE DELIBERATELY DOES NOT MOVE WITH IT (Wesley's call).** Gates and order of play are separate facts here (the 8/27 ruling) and we hold no real first-serve times for this stop, so **a day can read a gate LATER than its first serve** — Sep 18 shows Gates 12:00 PM against First serve 10:00 AM. That is the placeholder first serve, not the gate. Written on the constant so nobody "fixes" it; replace the stop with a real `eventSchedules` entry when the event team publishes one, and delete its line.
- **⚠ AND TWO COPY STRINGS WERE ASSERTING A RELATIONSHIP THAT NO LONGER HOLDS.** The schedule-section intro (*"Gates open an hour before first serve"*) and the concierge's answer to every schedule/gate question (*"Gates open 12:00 PM — about an hour before first serve each day"*) are only true of a **templated** gate. Both now just state the time when the event team set it, via `hasGatesOverride()` and a new required `ConciergeFacts.gatesTemplated`. Left unfixed, the chat bubble would have contradicted the order of play printed on the same page, at a live event.
- Verified on rendered pages, not by grep over source: Arizona renders **7× "Gates 12:00 PM"** plus the intro line and the KBYG accordion, with **0 occurrences of the hour-before claim**. **Controls unchanged — Las Vegas still 8/9/10 AM with the claim intact, the Nationals `-live` route byte-identical.** tsc clean, eslint at the NationalsLive `set-state-in-effect` baseline, `next build` green.
- **⚠ FOUND, NOT FIXED: the on-site `/today` screen shows no gate time for this stop at all**, before or after. `TodayPanel` renders gates off `proDays`, and Arizona has none. That is the screen someone opens standing in the lot; closing it needs the real per-day times, not a template.
- ⚠ Two method notes. `next build` **failed twice for reasons that were not the change**: killing the dev server truncated `.next/dev/types/routes.d.ts` mid-write (fixed by `rm -rf .next`), and an **untracked scratchpad probe from an earlier session does not compile** (`scratchpad/probe-platform-denied.ts`) — it fails the whole typecheck, so it has to be moved aside for a build. And `/events/*/today/` **404s on `next dev`** while serving 200 in production: that route is `force-static` with no `generateStaticParams`. Don't chase it.
...
- **Verified:** tsc clean, eslint clean on every changed file, `next build` green, rendered on a real server at 390 and 1440 (0 overflow, hero loaded), then on production: Grand Rapids + Charlotte upcoming, 18 past rows with Results ↗, rankings present, host-scoped redirects 308 to their sections with `/schedule/` on any other host still → `/events`.
- **⚠ FIXED AFTER THE FIRST DEPLOY: the tier alone is not the test.** `inferTier` files any sub-three-day / college / qualifier event as `challenger` to keep it off The Tour, so production showed the feed's one-day **"Utah Super Regional - College Pickleball Tour"** as a 500-point Challenger stop. The filter now also requires "Challenger" in the name — the same test the mapper uses for `isChallenger`.
- **9/19: domain attached.** `ppachallenger.com` + `www` are on the Vercel project (`vercel domains add … --scope bryce-pickleballs-projects`). DNS is GoDaddy (`ns17/18.domaincontrol.com`), A record still Flywheel's `151.101.194.159`, **no MX** so nothing hidden breaks. Vercel wants `A @ 76.76.21.21` + `A www 76.76.21.21`. **Domain expires 2026-10-30** — renew + auto-renew in GoDaddy. Bryce decides now vs after Charlotte (Sept 25–27).
- **Tanner Groff asked (9/19, DM) whether all the current pages move over.** Answered yes with the link, named the two deliberate omissions (stale RFP form, 2025 Showdown dates) and asked him for 2026 Showdown dates + the confirmed points level per stop.
- **Next:** Bryce — GoDaddy A records + renewal, Search Console export, WP form-lead export. Me — force the cert after DNS (`vercel certs issue ppachallenger.com --scope bryce-pickleballs-projects`), verify ten old URLs in a real browser, then Flywheel downgrade; Tanner's answers into the page; a live rankings source; internal Challenger event pages for the 1:1 post redirects.
...
- Email sent to Chris (9/2): fresh GitHub invite or transfer to Pickleball-Inc-Crew,
  Lovable bill, Cloud inventory. Proposed cutover Nov 9–15 (Worlds → Daytona gap).
- Next: Bryce accepts the Lovable invite; once repo access lands, audit
  `supabase/` and write the runbook.
